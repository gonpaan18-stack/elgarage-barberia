...
function ModalCargar({ hora, servicios, onConfirm, onClose }) {
  ...
  function confirmar() {
    ...
    onConfirm({ nombre: nombre.trim(), servicio: servicio.nombre, precio: servicio.precio, tipo: esFijo ? 'fijo' : 'nuevo' })
  }

  return (
    <div className="fixed inset-0 bg-black/70 flex items-end justify-center z-50" onClick={onClose}>
      <div className="bg-[#16181C] rounded-t-3xl w-full max-w-[430px] p-5 pb-8 flex flex-col gap-4" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between">
          <p className="font-bebas text-[22px] text-[#F3F0E9] m-0">Cargar turno · {hora}</p>
        ...
        <input autoFocus placeholder="Nombre del cliente" value={nombre} onChange={e => setNombre(e.target.value)}
        ...
        <div className="flex flex-col gap-1.5">
          {servicios.map(s => (
            <div key={s.nombre} onClick={() => setServicio(s)}
              className="flex justify-between items-center px-4 py-3 rounded-xl cursor-pointer border transition-colors"
              style={{ background: servicio.nombre===s.nombre ? '#1c1a17' : '#0E0F11', borderColor: servicio.nombre===s.nombre ? '#CE2434' : 'rgba(255,255,255,.08)', color: servicio.nombre===s.nombre ? '#F3F0E9' : '#9a9a97' }}>
